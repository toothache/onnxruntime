...
class MyDataReader(CalibrationDataReader):
    ...
    def get_next(self):
        if self.preprocess_flag:
            ...
            self.enum_data_dicts = iter([{"data": np.random.rand(1,3,60,100).astype(np.float32)} for i in range(self.datasize)])
...
def benchmark(model_path):
    ...
    session = onnxruntime.InferenceSession(model_path, sess_options=sess_options)
    input_name = session.get_inputs()[0].name

    total = 0.0
    runs = 10

    input_data = np.random.rand(1,3,60,100).astype(np.float32)
    # Warming up
...
def main():
    ...
    dr = MyDataReader(calibration_dataset_path)
